...
```{bash}
plink2 \
--covar ../filtering/hail/PCAclean.eigenvec \
--covar-col-nums 3,4,5,6 \
--logistic hide-covar \
--out ../result/panc/pancall \
...
```{r}
res <- fread("../result/panc/pancall.Pancreatitis.glm.logistic") %>% 
  arrange(P)

casefre <- fread("../result/panc/case.afreq")
colnames(casefre) <- make.names(colnames(casefre))

cntfre <- fread("../result/panc/cnt.afreq")
